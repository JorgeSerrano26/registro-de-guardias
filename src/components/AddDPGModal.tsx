...
export const AddDPGModal = ({ action }: Props) => {
    const { devs, dpgs } = useApp()
    const [date, setDate] = useState<DateRangeType | null>(null);
    const [open, setOpen] = useState(false);

    const handleOpen = () => setOpen(!open);

    const disabledDates = useMemo(() => dpgs?.map((d) => ({ startDate: d.startDate, endDate: d.endDate })), [dpgs])
            ...
            {open && <div id="crud-modal" role="dialog" tabIndex={-1} aria-hidden={!open} onClick={handleOpen}
                ...
                <div onClick={e => e.stopPropagation()} className="relative p-4 w-full max-w-md max-h-full">
                    <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <form className="p-4 md:p-5" action={action} onSubmit={() => { handleOpen(); console.log("submit") }}>
